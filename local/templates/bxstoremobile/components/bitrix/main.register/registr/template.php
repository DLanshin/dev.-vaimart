					...
					<div class="form_body">
						<?foreach($arFields as $FIELD):?>
							...
							<?endif;?>
									...
									<?switch ($FIELD){
										case "PASSWORD":?>
											<input size="30" type="password" id="input_<?=$FIELD;?>" placeholder="Придумайте пароль" name="REGISTER[<?=$FIELD?>]" required value="<?=$arResult["VALUES"][$FIELD]?>" autocomplete="off" class="password <?=(array_key_exists( $FIELD, $arResult['ERRORS'] ))? 'error': ''?>"  />

										<?break;
										case "CONFIRM_PASSWORD":?>
											<input size="30" type="password" id="input_<?=$FIELD;?>" placeholder="Подвердение пароля" name="REGISTER[<?=$FIELD?>]" required value="<?=$arResult["VALUES"][$FIELD]?>" autocomplete="off" class="confirm_password <?=(array_key_exists( $FIELD, $arResult['ERRORS'] ))? 'error': ''?>" />
										...
										<?case "PERSONAL_STREET":?>
											...
											<?break;?>
										<?case "EMAIL":?>
											<input size="30" type="email" id="input_<?=$FIELD;?>" placeholder="Ваш Email" name="REGISTER[<?=$FIELD?>]" <?=($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "required": "");?> value="<?=$arResult["VALUES"][$FIELD]?>" <?=$class?> id="emails"/>
										<?break;?>
										<?case "NAME":?>
											<input size="30" type="text" id="input_<?=$FIELD;?>" placeholder="<?=htmlspecialcharsbx($_REQUEST["REGISTER"]["NAME"])?>" name="REGISTER[<?=$FIELD?>]" <?=($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "required": "");?> value="<?=htmlspecialcharsbx($_REQUEST["REGISTER"]["NAME"])?>" <?=$class?>/>
										<?break;?>
										<?case "PERSONAL_PHONE":?>
											<input size="30" type="tel" id="input_<?=$FIELD;?>" name="REGISTER[<?=$FIELD?>]" placeholder="Номер телефона" class="phone_input <?=(array_key_exists( $FIELD, $arResult['ERRORS'] ))? 'error': ''?>" <?=($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "required": "");?> value="<?=$arResult["VALUES"][$FIELD]?>" />
										<?break;?>
										<?break;
										default:?>
											<?// hide login?>
											<input size="30" id="input_<?=$FIELD;?>" <?=(($FIELD == "LOGIN" && $arTheme["LOGIN_EQUAL_EMAIL"]["VALUE"] == "Y") ? 'type="hidden" value="1"' : 'type="text"');?> name="REGISTER[<?=$FIELD?>]" placeholder="Лоигн" value="<?=$arResult["VALUES"][$FIELD]?>" />
