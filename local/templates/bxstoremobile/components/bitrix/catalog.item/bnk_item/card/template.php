...
<div class="product-item">
	...
	<div class="product-item-title">
		...
		<? endif; ?>
        <?//= mb_substr($productTitle,0,10, 'utf-8')?>
		<?=$productTitle?>
		<? if ($itemHasDetailUrl): ?>
		</a>
		<? endif; ?>
					...
					if (!$haveOffers)
					{
						if ($actualItem['CAN_BUY'] && $arParams['USE_PRODUCT_QUANTITY'])
						{
							...
							<div class="product-item-info-container product-item-hidden" data-entity="quantity-block">
								<div class="product-item-amount js-row">
									<div class="product-item-amount-field-container  quantity js-quantity js-quantity-<?=$item['ID'];?> __empty" data-id="<?=$item['ID'];?>">
										<span class="product-item-amount-field-btn-minus no-select js-button-dec" id="<?=$itemIds['QUANTITY_DOWN']?>"></span>
										<input class="product-item-amount-field js-input-q inpdetal js-id-<?=$item['ID']?>"  id="<?=$itemIds['QUANTITY']?>" type="number"
                ...
                <script type="text/javascript">
                    jQuery(function ($) {
                        function updateQuantity(id, q) {
                            var row = $('.js-quantity-' + id).closest('.js-row');
                            if (q > 0) {
                                row.find('.js-quantity').removeClass('__empty');
                                row.find('.js-cart-add').hide();
                                row.find('.js-to-cart').show();
                            } else {
                                row.find('.js-quantity').addClass('__empty');
                                row.find('.js-cart-add').show();
                                row.find('.js-to-cart').hide();
                            }
                        ...
                        $('.js-button-dec').on('click', function () {
                            var id = $(this).closest('.js-quantity').data('id');
                            var input = $(this).closest('.js-quantity').find('.js-input-q');
                        ...
                        $('.js-button-inc').on('click', function () {
                            var id = $(this).closest('.js-quantity').data('id');
                            var input = $(this).closest('.js-quantity').find('.js-input-q');
                            var value = parseInt(input.val());
                            if (isNaN(value)) {
                        ...
                        $('.js-input-q').on('change', function (e) {
                            var id = $(this).closest('.js-quantity').data('id');
                            var q = $(this).val();
                            updateQuantity(id, q);
                        });
                        $('.js-cart-add').on('click', function (e) {
                            ...
                            var id = $(this).data('id');
                            var q = parseInt($(this).closest('.js-row').find('.js-input-q').val());
                            $(this).closest('.js-row').find('.js-input-q').val(q + 1);
